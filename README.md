...
│  ├─ RESEARCH-对标调研.md   同类工具（mdnice/墨滴/doocs/135等）体验对照与硬伤清单
...
| `npm test` | 全链行为测试 209 项（离线假 fetch 测 AI/生图/微信/备份/调度/剪贴板/表格渲染等，不产生任何真实请求） |
| `npm run verify` | 真实调用 AI 生成整篇文章，21 项质量断言（图槽位置/去味/HITL 零改动保证/渲染合规） |
| `npm run check <AppID> <Secret>` | 微信 API 权限探测，不产生任何线上写操作 |

## 已知边界

- 微信 API 无"定时发布"字段 → 定时靠本机调度器，到点推草稿 + 系统通知，**最终发布由人放行**；关机错过的发布任务不擅自补发，转「错过时间·等你决定」
- 未认证个人订阅号可能报 48001（无草稿 API 权限）→ 正式发布方式为写作页「复制富文本」+ 后台粘贴（发布页横幅实时指路，API 失败一键改道，定时任务走「提醒模式」）
- exe 未代码签名：新机器首跑可能被 SmartScreen 拦截（用户手册第一节「第 0 步」与排查表已给一键自救：更多信息→仍要运行）；DPAPI 密钥不随 exe 迁移 → 数据用设置页「导出/导入备份包」整体搬运（密钥绝不进包，新机重填一次 Key 即可）
